...
/**
 * General single threaded scan operator for babelfish data formats.
 */
@NodeInfo(shortName = "scan")
public class BFScanOperator extends Node implements BFOperatorInterface {
    ...
    public BFScanOperator(TruffleLanguage<?> language,
        ...
        this.bufferReader = new LuthBufferReader(catalog.getLayout(layout));
        this.tableName = layout;
        this.scanRepeatingNode = Truffle.getRuntime().createLoopNode(new ScanRepeatingNode(frameDescriptor, child));
    ...
    public class ScanRepeatingNode extends Node implements RepeatingNode {
        ...
        ScanRepeatingNode(FrameDescriptor frameDescriptor, BFOperator child) {
            TruffleRuntime runtime = Truffle.getRuntime();
