...
public class SSBQuery13 {
    ...
    static DynamicFilterFunction javafilterTypedUDF = new DynamicFilterFunction() {
        ...
        public boolean filter(DynamicRecord record) {
            ...
            return lo_quantity.asInt() >= 26 && lo_quantity.asInt() <= 35 && lo_discount.getValue() >= 500 && lo_discount.getValue() <= 700;
        }
    };

    public static LogicalOperator relationalQuery() {
        Scan scan1 = new Scan("table.sbb_date");
        Projection projection1 = new Projection(new FieldReference("d_yearmonthnum", Int_32.class), new FieldReference("d_weeknuminyear", Int_32.class), new FieldReference("d_datekey", Int_32.class));
        ...
        Selection selection1 = new Selection(
                new Predicate.And(
                        new Predicate.Equal<>(
                                ...
                                new FieldConstant<>(new Eager_Int_32(1994))),
                        new Predicate.Equal<>(
                                new FieldReference<>("d_weeknuminyear", Int_32.class),
                                new FieldConstant<>(new Eager_Int_32(6))))
                ...
                new Predicate.And(
                        ...
                        new Predicate.And(
                                new Predicate.GreaterEquals<>(
                                        ...
                                        new FieldConstant<>(new EagerNumeric(500, 2))),
                                new Predicate.LessEquals<>(
                                        new FieldReference<>("lo_discount", Numeric.class, 2),
                                        new FieldConstant<>(new EagerNumeric(700, 2))))));
    ...
    public static LogicalOperator pythonQuery() {
        ...
        PythonOperator selection2 = new PythonOperator(new PythonUDF(
                "lambda rec,ctx: (rec.lo_quantity>=26) and (rec.lo_quantity<=35) and (rec.lo_discount >= 5) and (rec.lo_discount<=7)"));
        scan2.addChild(selection2);
    ...
    public static LogicalOperator javaScriptQuery() {
        ...
        JavaScriptOperator selection2 = new JavaScriptOperator(new JavaScriptUDF("(record,ctx)=>{" +
                "return (record.lo_quantity>=26) &&  (record.lo_quantity<=35) && (record.lo_discount >= 5) && (record.lo_discount <= 7);" +
                "}"));
        scan2.addChild(selection2);
