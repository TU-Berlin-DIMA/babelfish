...
public class StringUppercaseNaive {


    static DynamicMapFunction stringUppercaseUDF = new DynamicMapFunction() {
        @Override
        public void map(DynamicRecord input, DynamicRecord output) {
            String text = input.getString("o_orderpriority");
            output.setValue("o_orderpriority", text.toUpperCase());
        }
    };

    ;

    public static LogicalOperator stringUppercasePython() {
        ...
        PythonOperator selection = new PythonOperator(new PythonUDF("def udf(rec,ctx):\n" +
                "\trec.o_orderpriority = rec.o_orderpriority.upper()\n" +
                "\treturn rec\n" +
    ...
    public static LogicalOperator stringUppercaseJava() {
        Scan scan = new Scan("table.orders");
        Projection projection = new Projection(new FieldReference("o_orderpriority", Text.class, 25));
        scan.addChild(projection);
        UDFOperator selection = new JavaDynamicUDFOperator(stringUppercaseUDF);
        projection.addChild(selection);
        Sink sink = new Sink.MemorySink();
        selection.addChild(sink);
        return sink;
    ...
    public static LogicalOperator stringUppercaseJavaScript() {
        Scan scan = new Scan("table.orders");
        Projection projection = new Projection(new FieldReference("o_orderpriority", Text.class, 25));
        scan.addChild(projection);
        JavaScriptOperator selection = new JavaScriptOperator(new JavaScriptUDF("(record,ctx)=>{" +
                "record.o_orderpriority = record.o_orderpriority.toUpperCase();" +
                "return record;" +
                "}"));
        projection.addChild(selection);
        Sink sink = new Sink.MemorySink();
    ...
    public static LogicalOperator getExecution(String language) {
        ...
        switch (language) {
            case "python":
                return stringUppercasePython();
            case "java":
                return stringUppercaseJava();
            case "js":
                return stringUppercaseJavaScript();
        }
